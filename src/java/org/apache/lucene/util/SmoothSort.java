...
// Taken from http://en.wikipedia.org/wiki/Smoothsort
// Modified for use with a Comparator
...
// by keeping these constants, we can avoid the tiresome business
  ...
  public static <C> void sort(C[] array, Comparator<? super C> c) {
    sort(array, 0, array.length-1, c);
  }

  public static <C> void sort(C[] m, int lo, int hi, Comparator<? super C> c) {
  ...
  private static <C> void sift(
      C[] m, int pshift, int head, Comparator<? super C> c) {
  ...
  private static <C> void trinkle(C[] m, int p, int pshift, int head,
                                  boolean isTrusty, Comparator<? super C> c) {
    ...
    while (p != 1) {
      ...
      if (!isTrusty && pshift > 1) {
        int rt = head - 1;
        int lf = head - 1 - LP[pshift - 2];
        if (c.compare(m[rt], m[stepson]) >= 0
            || c.compare(m[lf], m[stepson]) >= 0)
          break;
      }
